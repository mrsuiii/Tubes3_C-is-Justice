...
public class AES {
    ...
    public static byte[] getRoundKeys (byte[] key){
        ...
        for (int w = 4; w < 44; w++){
            ...
            if (w % 4 == 0){ // g function special untuk kelipatan 4
                // rotate
                byte temp = t[0];
                t[0] = t[1];
                t[1] = t[2];
    ...
    public static byte[] aes128_encrypt (byte[] block, byte[] key){
    ...
    public static byte[] aes128_decrypt (byte[] block, byte[] key){
        // Input: 16 byte block, 16 byte key (16 byte = 128 bits)
